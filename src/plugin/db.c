...
{
    int ret = sqlite3_prepare_v2(datbase_handle, "INSERT INTO device VALUES (NULL, ?1, ?2);", -1,
                                 &sql_statements[SQLS_INSERT_DEVICE], NULL);
    RETURN_ATM_ERROR(ret, "Error creating: insert into device");
    ret = sqlite3_prepare_v2(datbase_handle, "select id from device where name=?1 and address=?2;", -1,
                             &sql_statements[SQLS_SELECT_DEVICE], NULL);
    RETURN_ATM_ERROR(ret, "Error creating: select from device");
    ret = sqlite3_prepare_v2(datbase_handle, "INSERT INTO `temperature` (device_id,time,value) VALUES (?1,?2,?3);", -1,
                             &sql_statements[SQLS_INSERT_TEMP], NULL);
    RETURN_ATM_ERROR(ret, "Error creating: insert into temperature");
    ret = sqlite3_prepare_v2(datbase_handle, "INSERT INTO `humidity` (device_id,time,value) VALUES (?1,?2,?3);", -1,
                             &sql_statements[SQLS_INSERT_HUM], NULL);
    RETURN_ATM_ERROR(ret, "Error creating: insert into humidity");
    ret = sqlite3_prepare_v2(datbase_handle, "INSERT INTO `voc` (device_id,time,value,description) VALUES (?1,?2,?3,?4);", -1,
                             &sql_statements[SQLS_INSERT_VOC], NULL);
    RETURN_ATM_ERROR(ret, "Error creating: insert into voc");

    /* Used for testing. */
    ret = sqlite3_prepare_v2(datbase_handle, "select value from temperature where device_id=?1 and time=?2;", -1,
                             &sql_statements[SQLS_GET_TEMP], NULL);
    RETURN_ATM_ERROR(ret, "Error creating: select temperature");

    ret = sqlite3_prepare_v2(datbase_handle, "select value from humidity where device_id=?1 and time=?2;", -1,
                             &sql_statements[SQLS_GET_HUM], NULL);
    RETURN_ATM_ERROR(ret, "Error creating: select hum");
    
    ret = sqlite3_prepare_v2(datbase_handle, "select value from voc where device_id=?1 and time=?2;", -1,
                             &sql_statements[SQLS_GET_VOC], NULL);
...
int db_plugin_find_device(const char *name, const char *address, int *id)
{
    ...
    int ret = sqlite3_bind_text(sql_statements[SQLS_SELECT_DEVICE], 1, name, -1, SQLITE_STATIC);
    ret = sqlite3_bind_text(sql_statements[SQLS_SELECT_DEVICE], 2, address, -1, SQLITE_STATIC);

    while ( (ret = sqlite3_step(sql_statements[SQLS_SELECT_DEVICE])) == SQLITE_ROW) {
...
int db_plugin_insert_device(const char *name, const char* address)
{
    ...
    ret = sqlite3_bind_text(sql_statements[SQLS_INSERT_DEVICE], 1, name, -1, SQLITE_STATIC);
    if (ret != SQLITE_OK) {
        PRINT_ERROR("ERROR binding '%s': %s\n", name, sqlite3_errmsg(datbase_handle));
        return ATMOTUBE_RET_ERROR;
    }

    ret = sqlite3_bind_text(sql_statements[SQLS_INSERT_DEVICE], 2, address, -1, SQLITE_STATIC);
    if (ret != SQLITE_OK) {
...
void voc(unsigned long ts, float value)
{
    ...
    if (started) {
        ...
        sqlite3_bind_text(sql_statements[SQLS_INSERT_VOC], 4, "", -1, SQLITE_STATIC);
