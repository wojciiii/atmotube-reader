     ...
     static void *ble_connect_device (void *arg)
{
  ...
  gatt_connection =
    gattlib_connect (NULL, addr, BDADDR_LE_PUBLIC, BT_SEC_LOW, 0, 0);
  if (gatt_connection == NULL)
    {
      gatt_connection =
	gattlib_connect (NULL, addr, BDADDR_LE_RANDOM, BT_SEC_LOW, 0, 0);
      if (gatt_connection == NULL)
	{
  ...
  ret =
    gattlib_discover_primary (gatt_connection, &services, &services_count);
  if (ret != 0)
    {
  ...
  ret =
    gattlib_discover_char (gatt_connection, &characteristics,
  ...
  for (i = 0; i < characteristics_count; i++)
    {
      ...
      printf
	("characteristic[%d] properties:%02x value_handle:%04x uuid:%s\n", i,
...
static void
ble_discovered_device (const char *addr, const char *name)
{
  ...
  ret =
    pthread_create (&connection->thread, NULL, ble_connect_device,
		    connection);
  if (ret != 0)
    {
...
main (int argc, const char *argv[])
{
  ...
  pthread_mutex_lock (&g_mutex);
  ret =
    gattlib_adapter_scan_enable (adapter, ble_discovered_device,
				 BLE_SCAN_TIMEOUT);
  if (ret)
    {
