...
static void
setup_output (int counter)
{
  snprintf (&filenamebuffer[0], sizeof (filenamebuffer),
	    "test_db_plugin-%d.db", counter);
...
END_TEST static int
check_values (unsigned long time,
	      unsigned long tempval, unsigned long humval, float vocval)
{
